...
export const GptMessageSelectableImage = ({ imageUrl, onImageSelected }: Props) => {
  ...
  useEffect(() => {
    ...
    // es una imagen de HTML en la parte de JS
    const image = new Image();
    // Anonymous porque sino no se va a poder editar imagenes que no sean cross domain
    image.crossOrigin = 'Anonymous';
    image.src = imageUrl;
    originalImageRef.current = image;
    // una vez que se carga la imagen llama el proceso
    image.onload = () => {
      // canvas width y canvas height son de 1024
      ctx?.drawImage(image, 0, 0, canvas.width, canvas.height);
    }
  }, []);

  // cuando se empieza hacer click
  const onMouseDown = (
  ...
  const onMouseMove = (
    ...
    // Dibujar el rectangulo que es negro
    // ctx. fillRect(coords.x, coords.y, width, height);
    // limpia o borra el espacio
